...
Docker 컨테이너는 이미 프로덕션 환경에서 수년간 검증된 격리 기술이다. 파일 시스템, 네트워크, 프로세스가 모두 격리된다. 구체적으로 각 MCP 서버 컨테이너는 다음과 같은 제한을 받는다:

| 리소스 | 제한 |
|--------|------|
| CPU | 1개로 제한 |
| 메모리 | 2GB로 제한 |
| 파일 시스템 | 호스트 접근 **불가** (사용자가 명시적으로 마운트한 경우만 허용) |
| 네트워크 | L7 프록시를 통한 allowlist 방식 (허용된 대상만 통신 가능) |
