...
10페이지짜리 기술 문서와 같은 주제를 다룬 2문단짜리 요약을 임베딩해보면 어떻게 될까? 두 벡터는 **방향은 비슷하지만 norm(길이)에 차이가 생길 수 있다**. 더 근본적인 이유는 학습 단계에 있다 — Sentence-Transformers, OpenAI `text-embedding-3-*`, Cohere 같은 주류 텍스트 임베딩 모델은 **contrastive learning + cosine loss**로 학습되어, "의미가 비슷한 문장은 같은 방향을 가리키도록" 최적화되었다. 즉 의미 정보는 의도적으로 **방향에 인코딩** 되고, magnitude에는 의미 외 노이즈(문장 길이, pooling 방식, [CLS] 토큰의 활성도 등)가 끼어든다. 모델에 따라 이미 정규화된 벡터를 반환하기도 하지만, 정규화 여부와 무관하게 의미는 방향에 담긴다는 사실이 핵심이다.
