...
# 키보드 입력 대신, input.txt 파일 내용을 프로그램의 stdin으로 사용
./program < input.txt
...
flowchart LR
    subgraph MyMac["내 맥북"]
        CC["Claude Code"]
        Server["MCP Server"]
        CC <-->|"파이프<br>(내부 연결)"| Server
    end

    External["외부"] -.->|"❌ 접근 불가"| MyMac

    style MyMac stroke:#2E7D32,stroke-width:2px
...
```mermaid
flowchart LR
    subgraph Chulsoo["철수 맥북"]
        CC1["Claude Code"]
    end

    subgraph Younghee["영희 맥북"]
        CC2["Claude Code"]
    end

    subgraph CompanyServer["회사 서버"]
        Server["MCP Server<br>:8080 열림"]
    end
    ...
    CC1 -->|"HTTP"| Server
    CC2 -->|"HTTP"| Server
    Hacker -.->|"접근 시도 가능!"| Server

    style CompanyServer stroke:#E53935,stroke-width:2px
    style CC1 fill:#1565C0,color:#fff
    style CC2 fill:#1565C0,color:#fff
    style Server fill:#F57C00,color:#fff
    style Hacker fill:#E53935,color:#fff
```
