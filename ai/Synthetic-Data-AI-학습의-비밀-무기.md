...
**Synthetic Data(합성 데이터)** 는 실제 세계에서 수집한 것이 아니라 **인공적으로 생성된 데이터** 다. 실제 데이터의 통계적 특성을 모방하도록 설계되며, 일반적으로 개인정보 노출 위험을 *크게 낮춘다* — 다만 생성 방식·검증에 따라 위험 수준이 달라지므로 "실제 정보가 전혀 없다"는 단정은 부정확하다 (메모리제이션·재식별 가능성, 5.1절 참고).
...
| 개인정보 | 포함 가능 (보호 필요) | **노출 위험 감소** (단, 메모리제이션·재식별 위험은 잔존하므로 사후 검증 필수, 5.1절 참고) |
...
Synthetic Data도 마찬가지다. **통계적 특성**은 실제와 같도록 설계하며, **실제 개인을 직접 식별하기 어렵게 만드는 것이 목표**다. 다만 생성 방식에 따라 원본 정보 암기(memorization)·재식별(re-identification) 위험이 남을 수 있으므로 "실제 정보가 전혀 없다"는 단정은 피해야 한다 (5.1절 참고).
...
Synthetic Data는 통계적 패턴을 모방하므로 일반적으로 **재식별 위험이 크게 낮아진다**. 하지만 "실제 정보가 *전혀* 포함되지 않는다"는 단언은 위험하다 — 생성 모델은 학습 데이터를 통째로 **암기(memorization)**해 그대로 출력할 수 있고, 충분한 보조 정보가 있으면 합성 데이터에서도 원본 개인이 *재식별(re-identification)*될 가능성이 남는다. 따라서 GDPR·HIPAA 같은 규정을 자동으로 우회한다고 보면 안 되며, 다음 절차가 권장된다:

- **재식별 위험 평가** (membership inference, attribute inference 공격 시뮬레이션)
- **차분 프라이버시(Differential Privacy)** 노이즈를 생성 과정에 추가해 특정 개인의 기여를 수학적으로 숨김 ([NIST 가이드](https://www.nist.gov/blogs/cybersecurity-insights/differentially-private-synthetic-data) 참고)
- 익명화·토큰화 후처리 검증

요약하면 합성 데이터는 *개인정보 부재*가 아니라 *개인정보 노출 위험을 줄이는 기술*로 이해해야 하고, 규제 환경에서는 사후 검증이 필수다.
...
**해결책 — 단순히 "섞기"가 아니라 "누적(accumulate)"이 핵심**: 후속 연구인 [Gerstgrasser et al. (2024) "Is Model Collapse Inevitable?"](https://arxiv.org/abs/2404.01413)는 collapse를 피하는 핵심 조건을 더 정확히 제시했다 — 매 세대마다 합성 데이터로 실제 데이터를 *대체* 하면 collapse가 발생하지만, **실제 데이터를 폐기하지 않고 그대로 유지(accumulate)하면서 합성 데이터를 추가**하는 구조라면 model collapse를 피할 수 있다. 즉 "섞는 비율"보다 "원본이 잊히지 않게 보존되는가"가 결정적이다. 데이터 파이프라인을 설계할 때는 합성 데이터로 원본을 교체하지 말고, 합성을 *증강(augmentation)*으로만 다뤄야 한다.
