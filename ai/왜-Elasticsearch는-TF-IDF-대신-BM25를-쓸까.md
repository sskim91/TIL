...
> **본문 공식과의 미세한 차이** — 자세히 보면 Lucene의 tf는 `freq / (freq + k1 * ...)` 형태로, 2.2절 본문 공식의 분자에 있던 `(k_1 + 1)` 인수가 빠져 있다. 이는 [Lucene 6의 의도적 단순화](https://issues.apache.org/jira/browse/LUCENE-6818) 다 — `(k_1 + 1)`은 모든 문서·모든 쿼리 항에 동일하게 곱해지는 **상수** 라 점수의 절대값만 바꾸고 **랭킹 순위에는 영향을 주지 않기 때문** 이다. 따라서 explain 출력의 tf 값은 학술 공식 결과의 `1/(k_1+1)` 배지만, 정렬 결과는 완전히 동일하다.
