...
**"Vector DB"라는 제품 카테고리 자체는 비교적 최근(2017~2019)에 등장했지만, 그 뿌리인 벡터 표현과 유사도 검색은 1970년대 정보검색 연구까지 거슬러 올라간다.** 1972년 Spärck Jones의 TF-IDF, 1975년 Salton의 Vector Space Model이 그 출발점이고, 이후 DNA 시퀀스 검색·이미지 검색·추천 시스템에서 고차원 벡터 유사도 검색이 꾸준히 발전해 왔다. 그런데 왜 갑자기 2023년부터 *제품으로서의* Vector DB가 폭발적으로 뜨게 됐을까?
...
timeline
    ...
    section 태동기
        ...
        1972 : TF-IDF<br>(Spärck Jones)
        1975 : Vector Space Model<br>(Salton)
    section 발전기
        1990 : LSA<br>(잠재 의미 분석)
        1998 : LSH<br>(Locality-Sensitive Hashing)
        2000s : ANN 알고리즘 본격화
...
당시 1970년대 생물정보학의 대표적 해법은 벡터 저장소가 아니라 **Needleman-Wunsch(1970)** 같은 **sequence alignment(동적 계획법)** 알고리즘이었다. 즉 1970년대 DNA 검색이 Vector DB의 직접적 시초는 아니다 — 다만 "정확 일치를 넘어선 유사도 검색"이라는 수요를 일찍 키운 응용 분야였고, 이후 k-mer 빈도 벡터화 같은 기법이 도입되면서 점차 벡터 기반 검색과도 접점이 생겼다. Vector DB의 직접 계보는 앞 절에서 본 정보검색 분야의 Vector Space Model과 이후 ANN 인덱싱으로 이어진다.
...
| ANN (HNSW, IVF 등) | 인덱스·파라미터 의존, 전체 스캔보다 훨씬 빠른 sublinear 근사 (HNSW는 경험적으로 $O(\log n)$, IVF는 `nprobe` 등 파라미터에 따라 가변) | 95%+ (튜닝 가능) |
...
| **Annoy** | 2013 (Spotify, Erik Bernhardsson) | 경량, 읽기 최적화 |
