...
> V100은 2017년 세대 GPU라는 점을 감안해야 한다. 2026년 표준인 H100·B200으로 옮기면 같은 작업이 대략 **H100에서 10여 시간, B200에서는 수 시간** 으로 줄어든다 (FP16 기준 V100 대비 H100 ~3-4배, B200 ~10배 이상). 그래도 한 번의 쿼리에 시간 단위가 걸리는 건 변하지 않으므로, "수백만 문서 × 실시간 응답"이라는 조합에는 여전히 부적합하다는 결론은 그대로다.
