...
> **⚠️ 직렬화 포맷 보안 주의** — 위 예시처럼 단순 직렬화로 모델을 저장·로드할 때, **신뢰할 수 없는 출처의 직렬화 파일을 로드하면 임의 코드 실행(RCE) 취약점**이 될 수 있다. 외부에서 받은 모델 파일은 그대로 로드하지 말고 출처 검증 절차를 거쳐야 한다.
>
> 실무 권장 대안:
>
> | 용도 | 권장 포맷 | 이유 |
> |------|-----------|------|
> | scikit-learn / 일반 객체 | `joblib` (scikit-learn 공식 권장) | NumPy 배열 직렬화에 최적화. 다만 동일 보안 위험은 남으므로 출처 검증 필수 |
> | 딥러닝 가중치 (PyTorch / HF) | `safetensors` | **임의 코드 실행 불가능한 안전 포맷** — HuggingFace 표준 |
> | 크로스 프레임워크 배포 | `ONNX` | 코드 없이 가중치+그래프만 저장, 런타임 분리 |
>
> 학습용 코드에서는 그대로 가도 무방하지만, 서비스에 배포되는 모델은 처음부터 `safetensors`나 `joblib`로 저장하는 것이 안전하다.
