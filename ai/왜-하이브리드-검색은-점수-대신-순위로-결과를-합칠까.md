...
이게 의도된 설계다. $k = 60$은 **"상위권끼리는 비교적 평평하게 대우하고, 하위권으로 갈수록 기여도를 완만하게 깎는"** 균형 잡힌 값이다 (다음 표에서 보듯 k를 더 키우면 "보수적", 줄이면 "공격적"이 된다 — 60은 그 사이의 경험적 스윗 스팟이다). 왜 이런 평평함을 택할까? 상위권 10등 안에 들어온 문서들은 어느 쪽 검색기가 정답인지 **확실히 알 수 없기 때문** 이다. 두 검색기 모두에서 공통적으로 상위에 나타난 문서를 우선하되, 그 안에서의 순서 차이에는 덜 민감하게 굴겠다는 철학이다.
