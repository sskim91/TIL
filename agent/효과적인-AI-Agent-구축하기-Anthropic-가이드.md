...
| **포맷팅 인지 부담 최소화** | 모델이 추가로 "신경 써야 하는" 포맷을 피하라 — diff처럼 행 번호를 정확히 카운팅해야 하거나, JSON 문자열 안에 코드를 escape해 넣어야 하는 형식이 대표적이다. ⚠️ 이는 *JSON 스키마(Structured Outputs)를 피하라*는 뜻이 *아니다* — 오히려 스키마 강제는 도구 안정성의 핵심이며, 2026년 시점에는 권장 도구다. |
