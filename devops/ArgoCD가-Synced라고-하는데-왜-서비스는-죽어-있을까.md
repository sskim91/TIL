...
- **Matrix (Git × Cluster)** — "저장소에 있는 모든 앱 × 등록된 모든 프로드 클러스터"가 한 매니페스트로 표현된다. 클러스터를 하나 추가하면 그 클러스터에 앱 전체가 자동으로 깔린다. GitOps 노트 4.1절이 "GitOps가 빛나는 상황"으로 꼽은 멀티 클러스터가 실제로 이 지점에서 실현된다.
...
여기서 GitOps 노트 6.2절이 권장한 "App Repo와 Config Repo 분리"가 구체적인 메커니즘을 요구하게 된다. **ArgoCD는 이미지를 빌드하지 않는다.** Git에 적힌 태그를 읽어 apply할 뿐이다. 그러면 새로 빌드한 이미지 태그는 누가 Git에 적는가?
