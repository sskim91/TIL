...
카페에서 노트북으로 회사 DB에 접속한다고 가정해보자. MySQL(`--ssl-mode=PREFERRED` 기본), PostgreSQL(`sslmode=prefer` 기본) 모두 서버에 TLS가 설정되어 있으면 자동으로 시도하지만, **강제되지 않으면 평문으로 fallback**된다. 즉 서버 설정·인증서 검증을 신경 쓰지 않으면 공용 Wi-Fi에서는 평문 통신이 그대로 노출될 수 있다. 이 상태에서 어떤 위협이 가능할까?
