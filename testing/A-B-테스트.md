...
**통계적 유의성(Statistical Significance)** 은 관측된 차이가 단순한 표본 변동(우연)으로 설명되기 어려울 만큼 큰지를 판단하는 기준이다.

| 용어 | 의미 | 일반적 기준 |
|------|------|------------|
| **p-value** | 귀무가설(H₀: 두 그룹에 차이 없음)이 참이라고 가정했을 때, 관측된 결과 이상으로 극단적인 결과가 나올 확률 | < 0.05 |
| **신뢰구간** | 동일한 실험을 여러 번 반복했을 때 모수가 포함되리라 기대되는 범위 | 95% 신뢰구간 |
| **검정력(Power)** | 실제로 효과가 존재할 때 그 효과를 검출(귀무가설 기각)할 확률 = 1 − β | > 80% |

> **주의:** p-value는 흔히 "우연일 확률" 또는 "H₀가 참일 확률"로 오해되지만 둘 다 틀렸다. p-value는 H₀를 가정한 **조건부 확률**이며, H₀ 자체의 참/거짓 확률을 말하지 않는다. (참고: [NIST e-Handbook §1.3.1](https://www.itl.nist.gov/div898/handbook/prc/section1/prc131.htm))
...
```
필요 샘플 크기 = f(기존 전환율, 감지하고 싶은 변화량, 신뢰수준, 검정력)
```

두 비율 비교(z-test) 공식:

$$
n \approx \frac{(z_{\alpha/2} + z_\beta)^2 \cdot \big(p_1(1-p_1) + p_2(1-p_2)\big)}{(p_1 - p_2)^2}
$$

**95% 양측 신뢰수준(α=0.05), 80% 검정력(β=0.20)** 기준으로 계산하면:

| 기존 전환율 | 감지하려는 변화 | 필요 샘플 (그룹당) |
|------------|-----------------|-------------------|
| 5% | +1%p (→6%) | 약 8,200명 |
| 5% | +0.5%p (→5.5%) | 약 31,000명 |
| 10% | +1%p (→11%) | 약 14,700명 |

> 표본 크기는 차이의 제곱($\Delta^2$)에 반비례한다. 변화량이 절반이 되면 표본은 약 4배 필요하다. 정확한 수치는 R `power.prop.test` 또는 [Evan Miller 계산기](https://www.evanmiller.org/ab-testing/sample-size.html) 등으로 검증할 것. (참고: [NIST §2.4.2](https://www.itl.nist.gov/div898/handbook/prc/section2/prc242.htm))
