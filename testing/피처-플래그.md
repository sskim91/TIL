...
```java
@Component
@RequiredArgsConstructor   // Lombok: final 필드 생성자 자동 생성 (or 직접 생성자 작성)
...
```javascript
// 초기화 — JS SDK v3+ (2022년 GA): LDContext 사용
import * as LaunchDarkly from 'launchdarkly-js-client-sdk';

// v3는 user 객체 대신 context 사용. kind 필드 필수, custom은 평탄화됨
const context = {
    kind: 'user',
    key: 'user_123',
    email: 'user@example.com',
    plan: 'premium',     // v2의 custom: { plan: 'premium' } → 평탄화
};

const client = LaunchDarkly.initialize('YOUR_CLIENT_ID', context);
...
> **마이그레이션 주의:** SDK v2(legacy)는 `{ key, email, custom: {...} }` 형태의 `user` 객체를 받았지만, v3부터는 `kind`가 필수인 [`LDContext`](https://launchdarkly.com/docs/sdk/concepts/contexts)로 통일되었다. 옛 코드의 `custom`은 모두 최상위로 펼쳐 옮겨야 한다.
...
Unleash unleash = new DefaultUnleash(
    UnleashConfig.builder()
        .appName("my-app")
        .instanceId("my-instance-1")            // 필수: 인스턴스 식별자
        .unleashAPI("http://unleash-server/api")
        .apiKey("<client-api-token>")            // 필수: 환경별 client SDK 토큰
        .build()
);
