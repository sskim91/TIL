...
| 레벨 | 권장 비중 | 속도 | 비용 | 범위 |
|------|----------|------|------|------|
| **E2E** | 가장 적게 | 분 단위 | 높음 | 전체 시스템 |
| **Integration** | 중간 | 초 단위 | 중간 | 모듈 간 연동 |
| **Unit** | 가장 많이 | 밀리초 | 낮음 | 함수/클래스 |

> **70/20/10 비율 신화:** 흔히 "Unit 70 / Integration 20 / E2E 10"이라고 인용되지만, 이 수치는 [Mike Cohn의 원전(2009 *Succeeding with Agile*)](https://www.mountaingoatsoftware.com/blog/the-forgotten-layer-of-the-test-automation-pyramid)이나 [Martin Fowler의 글](https://martinfowler.com/bliki/TestPyramid.html)에 등장하지 않는다. 피라미드의 핵심은 **정확한 % 값이 아니라 상대적 분포**(아래로 갈수록 많이, 위로 갈수록 적게)와 **피드백 속도**다. 도메인·아키텍처에 따라 비율은 달라진다.
