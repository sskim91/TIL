...
- 모든 배포에 카나리 단계 필수
- **Kayenta**: Netflix와 Google이 **공동 개발**한 자동 카나리 분석(ACA) 엔진. Spinnaker 프로젝트의 일부로 오픈소스화([`spinnaker/kayenta`](https://github.com/spinnaker/kayenta))
- 통계적 비교(Mann-Whitney U test 등)로 카나리/베이스라인 차이 분석
