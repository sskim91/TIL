...
> ⚠️ **중요**: Linux에서 `O_NONBLOCK` 플래그는 **일반 디스크 파일과 블록 디바이스에는 효과가 없다**(항상 데이터가 "준비된" 상태로 간주됨). 논블로킹 read는 소켓·파이프·FIFO·터미널·시리얼 디바이스에서만 의미가 있다. 일반 파일을 진짜 비동기로 읽으려면 `aio_*`, `io_uring`, 또는 스레드 풀을 써야 한다.
...
def blocking_read(fd):
    ...
    data = os.read(fd, 1024)  # ⏳ 블로킹
    return data


# ========== 논블로킹 (Non-blocking) ==========
# 파이프를 만들어 논블로킹 read 시연 (소켓/파이프/FIFO에서만 의미 있음)
r, w = os.pipe()
os.set_blocking(r, False)  # 읽기 끝을 논블로킹으로 설정

def nonblocking_read(fd):
    try:
        # 즉시 리턴 (제어권 유지)
        return os.read(fd, 1024)  # ✓ 즉시 반환
    except BlockingIOError:
        # 데이터가 아직 없으면 EAGAIN 예외 발생 (즉시 리턴)
        return None

# 호출자는 즉시 제어권을 돌려받음
while True:
    data = nonblocking_read(r)
    if data:
        break
    print("아직 데이터 없음, 다른 작업 중...")
    # 실무에서는 selectors/epoll로 깨워주는 방식 사용 (busy loop 방지)
```
...
public class NonBlockingIO {
    public void readData() throws IOException {
        ...
        if (bytesRead > 0) {
            ...
            // 논블로킹 모드에서 아직 읽을 데이터가 없음
            System.out.println("아직 데이터 없음, 다른 작업 수행 가능");
        } else { // bytesRead == -1
            // ⚠️ -1은 "데이터 없음"이 아니라 end-of-stream — peer가 연결을 닫음
            System.out.println("연결 종료 — 채널 정리 필요");
            channel.close();
        }
    }
}
...
> ⚠️ **Java NIO `SocketChannel.read()` 반환값 함정**: `> 0`은 읽은 바이트 수, `0`은 논블로킹 모드에서 읽을 데이터 없음, `-1`은 **end-of-stream(연결 종료)**이다. `-1`을 "데이터 없음"으로 오해하면 끊긴 연결을 계속 select 대기하거나 리소스 누수가 발생한다.
...
// 비동기 + 논블로킹
while (true) {
    selector.select();  // ⚠️ 이 호출 자체는 블로킹 (이벤트 발생까지 대기). 논블로킹 폴링은 selectNow() 사용. 채널 I/O는 configureBlocking(false)로 논블로킹
...
graph TB
    ...
    style Note2 fill:#388E3C,color:#fff
```
...
# ❌ 잘못된 예 2: 여러 독립 작업을 직렬로 await
async def bad_async():
    # await 자체는 이벤트 루프를 블로킹하지 않음 (다른 task는 진행됨)
    # 다만 한 함수 안에서 독립적인 작업들을 한 줄씩 await하면
    # 병렬성을 못 살리고 순차 실행이 됨
    a = await task1()  # task1 완료까지 이 함수만 대기
    b = await task2()  # task2도 직렬로
    c = await task3()
    return a, b, c
...
> 📝 `await` 자체는 이벤트 루프를 블로킹하지 않는다 — 현재 코루틴만 일시 중단되고 다른 Task와 I/O는 계속 진행된다. 진짜 이벤트 루프를 막는 안티패턴은 `time.sleep()`, 동기 DB 드라이버 호출 등 **블로킹 동기 호출을 비동기 함수 안에서 직접 부르는 경우**(잘못된 예 1)다.
