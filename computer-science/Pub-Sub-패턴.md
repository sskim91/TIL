...
| **메시지 보존** | 앱 종료 시 유실 | **브로커마다 다름** ⚠️ |
| **장애 복구** | 불가능 | 보존되는 브로커만 가능 |
| **성능** | 매우 빠름 (ns~μs) | 상대적으로 느림 (ms) |
| **복잡도** | 낮음 | 높음 (인프라 운영 필요) |

> ⚠️ **메시지 보존 함정**: Out-of-Process라고 무조건 영속화되는 게 아니다. 브로커별로 차이가 크다.
> - **Kafka**: 디스크에 segmen 단위 저장, 보관 기간(retention) 동안 리플레이 가능
> - **RabbitMQ**: durable 큐 + persistent 메시지 옵션 켜야 디스크 저장
> - **Redis Pub/Sub**: **fire-and-forget — 활성 구독자 없으면 즉시 손실, 영속화 없음**
> - **Redis Streams**: Pub/Sub과 별개 자료구조로, 메시지 보존·재처리 지원
> - **AWS SNS**: 자체 보관 없음, SQS 같은 영속 큐를 구독시켜야 함
>
> Redis Pub/Sub을 Kafka처럼 생각하고 쓰면 메시지 유실 사고가 난다. "어떤 브로커가 보존하는가"는 항상 명시적으로 확인해야 한다.
