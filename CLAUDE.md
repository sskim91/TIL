...
`\n`은 mermaid에서 줄바꿈으로 해석되지 않고 텍스트로 그대로 출력된다.
...
```markdown
✅ Good: 문장 중간 (인라인)
B-Tree는 삽입/삭제 시 **항상** $O(\log n)$을 보장한다.
...
❌ Bad: 문장 중간에 블록 수식
B-Tree는 $$O(\log n)$$을 보장한다.  ← 어색함

❌ Bad: 일반 텍스트로 수학 표기
B-Tree는 O(log n)을 보장한다.  ← LaTeX 미사용
```
...
- [ ] Bold 처리 시 닫는 `**` 다음에 띄어쓰기가 있는가?
- [ ] 수식은 LaTeX로 작성했는가?
- [ ] 문단 단위로 설명했는가? (한 줄씩 끊지 않음)

---

## 10. Self-Check (문서 작성 후 필수)

**TIL 문서 작성 완료 후, Gemini 리뷰 전에 아래 항목을 스스로 점검하라.**

### Bold 규칙 점검

닫는 `**` 다음에 띄어쓰기가 있는지 확인하라. 바로 한글/영문이 붙으면 bold가 적용되지 않는다.

```markdown
❌ NG: **정식 지원(Stable)**된다.  (닫는 ** 다음에 바로 "된다"가 붙음)
✅ OK: **정식 지원(Stable)** 된다.  (닫는 ** 다음에 띄어쓰기)
```

**점검 방법:** 문서에서 `**` 패턴을 검색하여 닫는 `**` 다음 문자 확인.
