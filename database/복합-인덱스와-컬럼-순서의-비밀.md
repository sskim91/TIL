...
`EXPLAIN`의 `key_len`은 인덱스에서 **실제로 사용된 바이트 수**를 보여준다. 옵티마이저가 복합 인덱스의 어디까지 활용했는지 역산할 수 있다.

```sql
-- 인덱스: (status VARCHAR(20) NOT NULL, user_id BIGINT NOT NULL, created_at DATETIME NOT NULL)
-- charset: utf8mb4 (MySQL 8.0 기본값)

EXPLAIN SELECT * FROM orders
WHERE status = 'COMPLETED' AND user_id = 100;
```

**컬럼별 바이트 계산 (MySQL 8.0 / utf8mb4 / NOT NULL 기준):**

| 컬럼 | 타입 | 바이트 | 산출 근거 |
|------|------|--------|-----------|
| `status` | VARCHAR(20) | **81** | 20 × 4 (utf8mb4) + 1 (length prefix). 최대 바이트 80 ≤ 255이므로 prefix는 **1 byte** |
| `user_id` | BIGINT | **8** | 고정 8 bytes |
| `created_at` | DATETIME | **5** | MySQL 5.6.4+에서 5 bytes (이전엔 8 bytes) |

| key_len | 의미 |
|---------|------|
| 81 bytes | status만 사용 |
| 89 bytes | status + user_id 사용 |
| 94 bytes | 전체 인덱스 사용 |

> **주의할 점:**
> - **VARCHAR length prefix는 1 또는 2 bytes다.** 컬럼의 최대 가능 바이트 길이가 **255 이하면 1 byte**, **256 이상이면 2 bytes**. `VARCHAR(20)`은 charset과 무관하게 prefix 1 byte지만, `VARCHAR(100)` utf8mb4(=400 bytes)는 prefix가 2 bytes가 된다.
> - **charset에 따라 VARCHAR 크기가 달라진다.** 같은 `VARCHAR(20)`이라도 latin1/utf8mb3/utf8mb4 = **21 / 61 / 81 bytes**. (utf8mb3는 deprecated, MySQL 8.0 기본은 utf8mb4)
> - **DATETIME은 5.6.4를 분기점**으로 8 → 5 bytes로 줄었다. 운영 중인 MySQL 버전에 따라 key_len이 달라지므로 EXPLAIN을 분석할 때는 버전 확인이 먼저다.
> - **NULL 허용 컬럼은 +1 byte**가 추가된다. 위 예시에서 `status NULL`이라면 82 bytes.
