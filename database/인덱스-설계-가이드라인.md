...
> 인덱스는 기본적으로 오름차순(ASC)으로 생성된다. `ORDER BY ... DESC`를 사용하면 데이터베이스는 인덱스를 역순으로 스캔(Index Backward Scan)하여 정렬 작업을 생략한다.
>
> **단일 컬럼 정렬**이라면 Backward Scan만으로 충분해서 굳이 내림차순 인덱스를 만들 필요가 없다. 진짜 내림차순 인덱스(`CREATE INDEX ... ON items (registered_date DESC)`)가 결정적인 경우는 **혼합 정렬**(`ORDER BY a ASC, b DESC`)이다. 이 경우 ASC만으로 만든 인덱스로는 양방향을 동시에 만족시킬 수 없어 filesort가 발생한다.
>
> 단, **내림차순 인덱스(Descending Index)는 MySQL 8.0+에서만 실제로 작동한다.** MySQL 5.7 이하에서는 `DESC` 키워드가 문법적으로 허용되지만 무시되어 ASC 인덱스로 생성된다(`SHOW CREATE TABLE`로 확인 가능). 사용 중인 버전을 반드시 확인하자.
