...
### 3.5 정확히는 B-Tree가 아니라 B+Tree

> 실무에서는 흔히 "DB 인덱스는 B-Tree"라고 부르지만, **MySQL(InnoDB), PostgreSQL, Oracle 등 거의 모든 RDBMS가 실제로 사용하는 것은 B-Tree의 변형인 B+Tree** 다. 단순 명칭 차이가 아니라 range scan 성능을 결정짓는 구조적 차이가 있다.

| 구분 | B-Tree | B+Tree |
|------|--------|--------|
| 데이터 저장 위치 | Internal 노드 + Leaf 노드 모두 | **Leaf 노드에만** |
| Internal 노드 역할 | 키 + 데이터 + 자식 포인터 | **키 + 자식 포인터만** (라우팅) |
| Leaf 노드 간 연결 | 없음 | **Linked list로 연결** |
| 범위 검색 | 트리를 오르내리며 탐색 | 시작점 찾은 뒤 leaf chain 순회 |
| 같은 페이지 크기의 fanout | 작음 | **큼** (internal에 키만 있으니 더 많이 담김) |

이 구조 덕분에 B+Tree는 다음 두 가지 강점을 가진다.

1. **범위 스캔이 빠르다.** `WHERE created_at BETWEEN ...` 같은 쿼리는 시작 leaf만 찾으면 그 뒤로 linked list를 따라가며 순차 I/O로 읽어내려간다. 6.2절에서 "리프 노드를 순차적으로 읽는다"고 표현한 것도 바로 이 구조를 전제로 한다.
2. **트리 깊이가 더 얕다.** Internal 노드가 데이터를 안 들고 키만 들기 때문에 fanout이 커진다. 같은 16KB 페이지에 더 많은 자식 포인터를 담을 수 있어, 수억 건의 데이터도 깊이 3~4로 처리 가능하다.

이 글의 나머지 부분에서 "B-Tree"라고 부르는 것은 이 B+Tree 변형을 가리킨다고 이해하면 된다.
