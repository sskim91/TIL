...
**Clustering Factor (CF)** 는 Oracle CBO 통계에서 자주 등장하는 지표로, 인덱스 leaf entry를 키 순서대로 따라갈 때 **직전 항목과 다른 테이블 블록으로 점프한 횟수** 를 의미한다. 같은 블록 안에 연속된 키들이 모여 있으면 점프가 줄어들어 CF가 작아진다.
...
**평가 기준은 행 수 대비 비율이 아니라 두 끝값(블록 수, 행 수) 사이 어디에 있는지** 다.

| CF 값의 위치 | 품질 | 의미 |
|--------------|------|------|
| **테이블 블록 수에 근접** | Excellent | 인덱스 순서로 읽을 때 한 블록당 평균 1회 방문 → 거의 순차 I/O |
| 블록 수와 행 수 사이 중간 | Fair | 어느 정도 흩어져 있어 손익분기점이 좁아짐 |
| **테이블 행 수에 근접** | Poor | 한 행당 거의 한 번씩 새 블록 방문 → 사실상 랜덤 I/O |

정규화하면 다음과 같이 0(최선) ~ 1(최악) 사이 값으로 비교할 수 있다.

$$
\text{Clustering Score} = \frac{CF - \text{Blocks}}{\text{Rows} - \text{Blocks}}
$$

> **왜 "행 수 대비 비율"만으로는 부족한가?** 1억 행이 100만 블록에 담겨 있다면, CF의 이론적 최솟값은 100만(=1%)이고 이미 Excellent다. 반면 1억 행이 5,000만 블록에 담긴 좁은 테이블이라면 CF 100만이 별 의미가 없다. 즉, 절대 % 임계값보다 **블록 수와의 거리**가 본질이다.
