...
| 방식 | B-Tree 구조 | 특징 |
|------|-------------|------|
| 복합 인덱스 | `(name, email)` 모두 키 | **leftmost prefix 원칙**으로 `name` 또는 `(name, email)` 검색에만 효율적. `email` 단독 검색은 인덱스 활용 불가 |
| Include Index | `name`만 키, `email`은 leaf에만 저장 | 인덱스 크기 절약, `email` 검색 불가 (검색용 아님) |

Include Index는 검색에 사용되지 않지만 조회가 필요한 컬럼을 효율적으로 저장한다. **키가 적어 B-Tree 깊이도 얕아지고, 비키 컬럼이 변경되어도 인덱스 재정렬이 일어나지 않아 쓰기 비용도 줄어든다.** SQL Server에서는 이 기능이 2005년부터 **Included Columns** 라는 이름으로 있었고, **MySQL은 8.0 기준으로도 INCLUDE를 지원하지 않는다.** MySQL에서는 모든 컬럼을 복합 인덱스 키로 넣어야 하므로, 비키 컬럼까지 정렬에 참여하는 오버헤드를 감수해야 한다.
